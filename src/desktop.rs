...
pub fn export_data_dir(project_root: &Path) -> PathBuf {
    project_root.join("exports").join("share")
}

pub fn export_app(project_root: &Path, app: &AppRecord) -> Result<ExportReport> {
    remove_export(project_root, &app.app_id)?;

    let source_share = state::absolute(project_root, &app.app_dir)
        .join("export")
        .join("share");
    ...
    let export_share = export_data_dir(project_root);
    fs::create_dir_all(&export_share)
        .with_context(|| format!("create export data dir {}", export_share.display()))?;

    let flatpak_bin = project_root.join("bin").join("flatpak");
    ...
    exported_paths.sort();
    state::write_export_record(project_root, &app.app_id, &exported_paths)?;
    refresh_export_caches(project_root)?;

    report.files = exported_paths.len();
    Ok(report)
}

pub fn remove_export(project_root: &Path, app_id: &str) -> Result<()> {
    let export_share = export_data_dir(project_root);
    let mut parents = Vec::new();

    for rel in state::read_export_record(project_root, app_id)? {
    ...
    state::remove_export_record(project_root, app_id)?;
    refresh_export_caches(project_root)?;
    Ok(())
}

pub fn refresh_export_caches(project_root: &Path) -> Result<()> {
    let export_share = export_data_dir(project_root);
    let applications = export_share.join("applications");
    ...
    let hicolor = export_share.join("icons").join("hicolor");
    if hicolor.is_dir() {
...
[Icon Theme]
Name=Hicolor
Comment=Fallback icon theme for project-local Flatpak exports
...
#[cfg(test)]
mod tests {
    use super::{desktop_exec, exec_tail_after_command};
    use std::path::Path;
