...
impl HostFilesystem {
    ...
    fn from_metadata_with_mount_points(
        ...
        Self::from_metadata_with_xdg_dirs(
            metadata,
            user,
            home,
            project_root,
    ...
    pub fn write_xdg_user_dirs_config(&self, config_home: &Path) -> Result<()> {
        ...
        let mut data =
            String::from("# Generated by freebsd-flatpak for the application sandbox.\n");
        for (label, key) in self.xdg_dirs.entries() {
            if let Some(path) = self.sandbox_dir_for_xdg_label(label) {
                let value = escape_xdg_config_value(&path.display().to_string());
    ...
    fn sandbox_dir_for_xdg_label(&self, label: &str) -> Option<PathBuf> {
        let host_path = self.xdg_dirs.get(label)?;
        let canonical_host_path =
            fs::canonicalize(host_path).unwrap_or_else(|_| host_path.to_path_buf());
        self.grants
            .iter()
            .filter_map(|grant| {
                grant
                    .map_host_path(host_path)
                    .or_else(|| grant.map_host_path(&canonical_host_path))
                    .map(|sandbox_path| (grant.host_path.components().count(), sandbox_path))
            })
...
impl GrantBuilder {
    fn apply_permission(&mut self, permission: &FilesystemPermission) -> Result<()> {
        ...
        match base {
            ...
            | "xdg-public-share" | "xdg-templates" | "xdg-videos" => {
                let Some(host_base) = self.xdg_dirs.get(base) else {
                    self.warnings
                        .push(format!("unsupported XDG filesystem permission: {base}"));
                    return Ok(());
                };
                if host_base == &self.home {
                    self.warnings
                        .push(format!("disabled XDG filesystem permission: {base}"));
                    return Ok(());
                }
...
impl XdgUserDirs {
    ...
    fn load(home: &Path) -> Self {
        ...
        Self::load_from_config_home(home, &config_home)
    }

    fn load_from_config_home(home: &Path, config_home: &Path) -> Self {
        let mut dirs = Self::defaults(home);
        ...
        for line in data.lines() {
            ...
            match key.trim() {
                ...
                "XDG_TEMPLATES_DIR" => dirs.templates = path,
    ...
    fn get(&self, name: &str) -> Option<&PathBuf> {
        match name {
            ...
            "xdg-templates" => Some(&self.templates),
    ...
    fn entries(&self) -> [(&'static str, &'static str); 8] {
        [
