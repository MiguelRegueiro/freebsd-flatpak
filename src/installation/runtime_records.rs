...
pub fn list_runtimes(paths: &Installation) -> Result<Vec<RuntimeRecord>> {
    ...
    if let Some(duplicate) = runtimes
        .windows(2)
        .find(|records| records[0].runtime_ref == records[1].runtime_ref)
    {
        bail!(
            "duplicate installed runtime ref: {}",
            duplicate[0].runtime_ref
        );
    }
    Ok(runtimes)
}
...
pub fn list_runtime_deployments(paths: &Installation) -> Result<Vec<RuntimeRecord>> {
    ...
    collect_runtime_deployments(paths, &paths.runtimes(), &mut deployments)?;
...
fn collect_runtime_deployments(
    paths: &Installation,
    directory: &Path,
    deployments: &mut Vec<RuntimeRecord>,
) -> Result<()> {
    if !directory.is_dir() {
        return Ok(());
    }
    if let Some(runtime) = runtime_deployment_from_path(paths, directory)? {
    ...
    for entry in fs::read_dir(directory).with_context(|| format!("read {}", directory.display()))? {
        ...
        if entry.file_type()?.is_dir() {
            collect_runtime_deployments(paths, &entry.path(), deployments)?;
        }
    }
    Ok(())
}
...
pub fn remove_runtime_record(paths: &Installation, runtime_ref: &str) -> Result<()> {
    ...
    match fs::remove_file(&path) {
        ...
        Err(error) => Err(error).with_context(|| format!("remove runtime record {runtime_ref}")),
    }
}
...
pub(super) fn read_runtime_path(path: &Path) -> Result<RuntimeRecord> {
    ...
    Ok(RuntimeRecord {
        ...
        explicitly_installed: required(&values, "explicitly_installed")?
            .parse()
            .context("invalid explicitly_installed")?,
